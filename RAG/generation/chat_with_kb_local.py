# coding:utf-8
# @File  : chat_with_kb_local.py
# @Author: ganchun
# @Date  :  2025/04/23
# @Description: 基于知识库的对话系统，支持命令行和Web界面
...
from RAG.retrieval.contextual_rewrite import retrieve_from_knowledge_base, list_knowledge_bases, rerank

# 配置参数
LLM_API_URL = "http://localhost:9997/v1"
LLM_MODEL_UID = "qwen2.5-instruct"  # 对话使用的大模型
CHAT_HISTORY_DIR = r"F:\StrivingRendersMeCozy\DeepLearning\ERAG\Data\Conversation"  # 对话历史保存路径
...
class ChatSession:
    ...
    def __init__(self, kb_name: str, session_id: Optional[str] = None):
        """
        初始化聊天会话
        ...
        self.client = OpenAI(api_key="not empty", base_url=LLM_API_URL)
...
def chat_command_line(kb_name: str):
    """命令行聊天界面"""
    print(f"正在使用知识库: {kb_name}")
    print("输入'q'或'quit'退出对话\n")

    session = ChatSession(kb_name)
    ...
    while True:
        ...
        if references:
            print("\n参考来源:")
            for i, ref in enumerate(references, 1):
                print(f"[{i}] {ref.get('标签', [])} - {ref.get('总结', '')[:100]}...")
            print()


def chat_web_interface(kb_name: str):
    """Gradio Web聊天界面"""
    session = ChatSession(kb_name)
    references_store = []
    ...
    with gr.Blocks(title=f"知识库对话 - {kb_name}") as demo:
        gr.Markdown(f"# 基于知识库的对话系统\n\n当前知识库: **{kb_name}**")
...
def main():
    parser = argparse.ArgumentParser(description="基于知识库的对话系统")
    ...
    # 根据模式选择界面
    if args.mode == "cli":
        chat_command_line(args.kb)
    else:
        chat_web_interface(args.kb)
